...
// Assign secret angels (derangement)
const ids = RAW_CAMPERS.map(c => c.id)
const shuffled = createDerangement(ids)
const CAMPERS = RAW_CAMPERS.map((c, i) => ({
    ...c,
    secretAngelTargetId: shuffled[i],
    friends: [],
    emergencyContact: { name: `${c.name.split(' ')[1]} Sr.`, phone: '03-12345678', relation: 'Parent' },
