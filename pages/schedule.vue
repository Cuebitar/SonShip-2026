      ...
      <div v-else>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div class="lg:col-span-2 relative">
            ...
            <!-- 如果被锁住，加上 blur 和禁止点击的效果 -->
            <div :class="[
              isCurrentDayLocked ? 'blur-md opacity-30 pointer-events-none select-none' : 'opacity-100',
              'space-y-3 transition-opacity duration-150 ease-in-out'
            ]">
            ...
            <!-- 未解锁时的浮层提示 -->
            <div v-if="isCurrentDayLocked" class="absolute inset-0 flex flex-col items-center justify-center z-10 mt-10 lg:mt-0">
              <div class="bg-dark/80 backdrop-blur-sm p-6 rounded-xl border border-primary/30 text-center shadow-lg shadow-black/50">
...
import { MapPin, CalendarCheck, AlertCircle, Lock } from 'lucide-vue-next' // 新增 Lock icon
...
// 配置每天的解锁时间 (这里强制设置为东八区大马时间早上 6:00)
// ⚠️ 请确保 key 的名字和你 scheduleStore.getDays() 返回的格式一模一样！
// 如果你的 days 返回的是日期（如 '2026-08-28'），请将下面的 key 换成 '2026-08-28' 等。
...
// 判断当前选中的天数是否被锁住
const isCurrentDayLocked = computed(() => {
  const unlockTime = unlockSchedules[activeDay.value]
  // 如果找不到对应时间的配置，默认放行（开放）
  if (!unlockTime) return false 
  // 当前时间如果小于解锁时间，就返回 true（代表被锁）
...
onMounted(async () => {
  ...
  // 设定一个计时器，每 10 秒更新一次当前时间
  // 这样如果用户停留在页面上到了早上 6 点，页面会自动解锁，不需要刷新
