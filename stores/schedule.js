...
import { useDb } from '~/composable/firebase' // 确保这里的路径和你之前的一致

export const useScheduleStore = defineStore('schedule', () => {
  const schedule = ref([])
  const bookings = ref({}) // { userId: [scheduleId, ...] }
  const loaded = ref(false) // 记录是否已经从 Firebase 拉取过数据

  // === 从 Firebase 拉取行程表的初始化函数 ===
  async function initSchedule() {
    if (import.meta.server) return false
    if (loaded.value) return true // 如果已经加载过，就不重复请求，节省流量
    ...
    try {
      ...
      const fetchedSchedules = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }))
      
      // 按照日期和时间排序
      fetchedSchedules.sort((a, b) => new Date(`${a.date} ${a.time}`) - new Date(`${b.date} ${b.time}`))
      
      schedule.value = fetchedSchedules
      loaded.value = true
    } catch (error) {
  ...
  function getDays() {
    // 动态提取所有的 Day (比如 'Day 1', 'Day 2') 并去重
    return [...new Set(schedule.value.map(s => s.day))]
  }
  ...
  return { schedule, bookings, loaded, initSchedule, getDays, getByDay, getUserBookings, isBooked, book, unbook }
})